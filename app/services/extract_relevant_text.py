...
def extract_relevant_text(prompt, path_of_files, top_n=100, algorithm_name="tf-idf"):
    ...
    for file_path in path_of_files:
        # Check if the file path is a URL (URL-based download)
        if file_path.startswith("http"):
            # Create a temporary file to save the downloaded file
            temp_file = tempfile.NamedTemporaryFile(delete=False)
            downloaded_file = download_file(file_path, temp_file.name)
            if downloaded_file:
                # Attempt to detect MIME type and append correct file extension if missing
                ...
                if extension:
                    ...
                    file_path = new_temp_file  # Update the path to the renamed temporary file
                else:
        ...
        # If the file was downloaded, delete the temporary file after processing
    ...
    # Search using the selected algorithm
